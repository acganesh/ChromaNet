...
#    return K.mean(K.binary_crossentropy(y_true,y_pred)*weightsPerTaskRep, axis=-1)

lr, decay, p = 0.001, 8e-7, 0.5
...
checkpointer = ModelCheckpoint(filepath="weights/weight.{epoch:02d}-{val_loss:.2f}_"+str(lr)+"_NewWeighting_Nadam.hdf5", verbose=1, save_best_only=False,save_weights_only=False)
